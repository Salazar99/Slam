...
public:
   ...
   */
  Template(slam::Trace *trace, DTLimits limits);
  ...
  /** \brief evaluates the robustness of the assertion generated by loading the propositions in the template
   * \param robustness type of robustness to evaluate
   * \param time trace instant on which to evaluate the assertion
   */ 
  float evaluate_robustness(R_types robustness, size_t time);
   ...
   */
  size_t getNumPlaceholders(slam::Location where);
...
public:
  ...
  void fillRobustness(RobustnessMatrix &rob, bool savePoints = false);
