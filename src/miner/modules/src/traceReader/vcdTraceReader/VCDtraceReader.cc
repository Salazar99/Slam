...
Trace *VCDtraceReader::readTrace(const std::string file) {
  ...
  //change the name of this 'run' of slam (used when printing/dumping stats), only if the user did not already define a name, same thing for the scope
  if (hs::name == "") {
    if (clc::selectedScope == "") {
  ...
  //slam dose not support logic types larger than 64 bits
  std::vector<std::string> gt64names;
  for (auto &n_vv : _nameToValues) {
    if (n_vv.second.size() > 64) {
